		 ...
		 (let ((current-prec (op-prec (car head)))
		   ...
		   (let ((gottok (next-token)))
		     (cond
		       ...
		       ((null head) (unget-token gottok) (return-from exit (parse-prim)))
		       ((operatorp gottok)
			(let* ((gottok-2 (let ((n (next-token))) (unget-token n) n))
			       (have-operand (not (non-operandp gottok-2))))
			  (do ((op (car head) (setq head (cdr head)) (car head)))
			      ((or (null op) (not (eq current-prec (op-prec op)))
				   (not (member (op-assoc op) '(fx fy))))
			       (unget-token gottok))
			    (if have-operand
				(case (op-assoc op)
				  (fx (return-from exit `(struct ,(op-atom op) ,(parse-sub next-head))))
				  (fy (return-from exit `(struct ,(op-atom op) ,(parse-sub current-head)))))))))
		     ...
		     (cond ((separatorp gottok) (unget-token gottok) (return-from exit operand-1))
			   ((operatorp gottok)
				...
				(if (and (eq (car gottok) 'atom) (eq (cdr gottok) (op-atom op)))
				    (if have-operand
					(case (op-assoc op)
					  (xfy	(return-from exit `(struct ,(op-atom op) ,operand-1 ,(parse-sub current-head))))
					  (xfx	(return-from exit `(struct ,(op-atom op) ,operand-1 ,(parse-sub next-head))))
					  (yfx
...
(defconstant variablep atom)
(defconstant termp listp)
...
(defun compile-head (head vartable))

(defun compile-body (body vartable)
  (setq body (flatten-comma body)))
		   ...
		   (let ((inst (car code)))
		     (case (car inst)
		       ...
		       (unify-local-value-permanent (let ((y (cadr inst)))
						      (case mode
							(read (unify (addr+ E y 1) S))
							(write (let ((addr (deref (addr+ E y 1))))
								 (if (addr< addr H)
								     (setf (heap H) (heap addr))
								     (progn
								       (setf (heap H) (cons 'ref H))
		       ...
		       (switch-on-constant (destructuring-bind (n t) (cdr inst)
					     ))
		       (switch-on-structure (destructuring-bind (n t) (cdr inst)
					      ))
		       (neck-cut )
